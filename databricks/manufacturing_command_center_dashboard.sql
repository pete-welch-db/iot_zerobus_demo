...
SELECT event_time, machine_id, temp_c
FROM vw_machine_telemetry_live
ORDER BY event_time DESC;

-- 2) Live Operations: vibration trend
SELECT event_time, machine_id, vibration_mm_s
FROM vw_machine_telemetry_live
ORDER BY event_time DESC;

-- 3) Live Operations: throughput trend
SELECT event_time, machine_id, throughput_cpm
FROM vw_machine_telemetry_live
ORDER BY event_time DESC;

-- 4) Live Operations: RPM trend
SELECT event_time, machine_id, rpm
FROM vw_machine_telemetry_live
ORDER BY event_time DESC;

-- 5) Live Operations: motor current trend
SELECT event_time, machine_id, current_amps
FROM vw_machine_telemetry_live
ORDER BY event_time DESC;

-- 6) Live Operations: humidity trend
SELECT event_time, machine_id, humidity_pct
FROM vw_machine_telemetry_live
ORDER BY event_time DESC;

-- 7) Health and risk KPI snapshot
SELECT
  machine_id, oee_pct, availability_pct, performance_pct, quality_pct,
  anomaly_score, prob_fault_next_5m, window_end
FROM vw_machine_health
QUALIFY row_number() OVER (PARTITION BY machine_id ORDER BY window_end DESC) = 1;

-- 8) Loss analysis by machine (stacked bar data)
SELECT
  machine_id, window_end,
  time_in_run_s, time_in_stopped_s, time_in_fault_s
FROM vw_machine_health
ORDER BY window_end DESC, machine_id;

-- 9) At-risk machine ranking
SELECT
  machine_id, prob_fault_next_5m, anomaly_score, oee_pct, window_end
FROM vw_machine_health
QUALIFY row_number() OVER (PARTITION BY machine_id ORDER BY window_end DESC) = 1
