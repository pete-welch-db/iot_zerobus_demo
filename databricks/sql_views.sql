-- Parameterize catalog/schema for your workspace before execution.
-- Example:
-- USE CATALOG main;
...
WITH anomaly_latest AS (
  SELECT machine_id, event_time, anomaly_score, is_anomaly
  FROM (
    SELECT
      machine_id,
      event_time,
...
fault_pred_latest AS (
  SELECT machine_id, event_time, prob_fault_next_5m, predicted_fault_next_5m
  FROM (
    SELECT
      machine_id,
      event_time,
